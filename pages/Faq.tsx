...
const AccordionItem: React.FC<{
  ...
  return (
    <div
      className={`border rounded-2xl bg-white mb-4 overflow-hidden transition-all duration-300 ${isOpen ? "border-brand-cyan shadow-md" : "border-zinc-200 hover:border-zinc-300"}`}
    >
      <button
      ...
      >
        <span
          className={`font-bold text-lg pr-4 ${isOpen ? "text-brand-navy" : "text-zinc-700"}`}
        >
          {q}
        </span>
        <div
          className={`mt-1 p-1 rounded-full transition-colors ${isOpen ? "bg-brand-cyan/10 text-brand-cyan" : "bg-zinc-100 text-zinc-400"}`}
        >
          {isOpen ? (
      ...
      >
        <div className="pt-2 border-t border-zinc-100">
          <p
            className="text-zinc-600 leading-relaxed mt-4"
...
const Faq: React.FC = () => {
  ...
  const currentCategoryData = faqCategories.find(
      ...
      <div className="bg-brand-navy text-white py-24 px-4 text-center border-b border-zinc-800 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-zinc-300 text-xl max-w-2xl mx-auto font-light leading-relaxed">
            Everything you need to know about the FinWorld ecosystem, security
            protocols, and investment structure.
          </p>
        </div>
      </div>
          ...
          <div className="grid lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-4">
              <div className="bg-white rounded-3xl p-4 shadow-lg border border-zinc-100 sticky top-32">
                <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-widest px-4 py-3">
                  Categories
                </h3>
                <div className="space-y-2">
                  {faqCategories.map((cat) => (
                    ...
                    >
                      <cat.icon
                        className={`w-5 h-5 ${activeCategory === cat.id ? "text-brand-cyan" : "text-zinc-400"}`}
                      />
                      {cat.title}
                    </button>
                  ))}
      ...
      <Section className="bg-zinc-50 border-t border-zinc-200">
        <div className="bg-white rounded-[2.5rem] p-8 md:p-12 text-center shadow-xl border border-zinc-100 max-w-4xl mx-auto">
          ...
          <p className="text-zinc-600 mb-8 max-w-xl mx-auto">
            Our support team is available to discuss your specific security
            needs and legacy planning goals.
          </p>
          ...
          {/* <a href="mailto:mark@finworld.us" className="px-8 py-3 bg-brand-navy text-white rounded-full font-bold hover:bg-brand-dark transition-colors shadow-lg">
            ...
            <a
              href="tel:5154443518"
              className="h-12 px-8 bg-brand-navy flex items-center justify-center gap-3 
            ...
            >
              ...
              <span>515-444-3518</span>
            </a>

            <Link
              to="/contact"
              className="h-12 px-8 bg-white border border-zinc-300 text-brand-navy 
               rounded-full font-bold flex items-center justify-center
               hover:border-brand-navy transition-colors"
            >
              Contact Sales
            </Link>
          </div>
